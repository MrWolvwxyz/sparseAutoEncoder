...
for line in sentences:
    ...
    if ( valid_sentence and ( len( line.split() ) > 1 )
               and ( string.find( string.ascii_uppercase, line[ 0 ] ) != -1 ) ):
...
new_set = [ training_set[ i ].get_size() for i in range( len( training_set ) ) ]

new_set = sorted( new_set, reverse = True )
